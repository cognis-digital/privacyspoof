...
How **privacyspoof** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `privacyspoof` lives in the **Geospatial / GEOINT** cluster.
...
| from | relation | to |
|---|---|---|
| `privacyspoof` | JSON findings compose with | [`locateanything`](https://github.com/cognis-digital/locateanything), [`geolens`](https://github.com/cognis-digital/geolens), [`geoaoi-pro`](https://github.com/cognis-digital/geoaoi-pro) |
| `privacyspoof` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `privacyspoof` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `privacyspoof` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the geospatial / geoint cluster.** `privacyspoof` output feeds the next tool:
```bash
privacyspoof ... --format json > out.json          # this tool's findings (see `privacyspoof --help`)
...
privacyspoof ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
privacyspoof ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
privacyspoof ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `privacyspoof`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `privacyspoof` slots
